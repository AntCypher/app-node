...
export default function NavigationMenu({ menuItems, className }) {
  ...
  function renderMenu(items, isSubMenu = false) {
    return (
      <ul className={cx(isSubMenu ? 'sub-menu' : 'menu')}>
        {items.map((item) => {
          ...
          return (
            <li key={id} className={cxFromWp(cssClasses, 'menu-item', { 'menu-item-child': children.length })}>
